...
class AuthTest(PluginBase):
    ...
    def test_unauthenticated_signedout_page(self):
        ...
        # TODO: plugin testhook is nested inside the plugin!
        self.wait_for_element_with_text('', 'You are signed out of KBase.')

        # Make sure the identity provider log out links are available with the correct links
        google_sign_out_link = self.wait_for_element_with_text('', 'Log out from Google')
        ...
        orcid_sign_out_link = self.wait_for_element_with_text('', 'Log out from ORCiD')
        ...
        globus_sign_out_link = self.wait_for_element_with_text('', 'Log out from Globus')
