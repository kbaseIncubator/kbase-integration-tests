...
def check_knockout_usage(dir_to_check, show_files=False, omit_pattern=None):
    print('')
    print('Checking for knockoutjs usage...')
    if omit_pattern is not None:
        print(f'Using omit pattern {omit_pattern}')
            ...
            for line_number, line in enumerate(lines):
                ...
                if 'html:' in line:
                    ...
                    if re.search('^(\s)*//', line):
                        comment_count += 1
                        handled = True
                    # check if safe (prev line contains a comment with xss safe)
                    elif prev_line is not None and re.search('^(\s)*//\s*xss\s*safe', prev_line):
                        safe_count += 1
                        handled = True
...
def check_react_usage(dir_to_check, show_files=True, omit_pattern=None):
    ...
    if VERBOSE or lines_to_investigate_count > 0:
        ...
        if lines_to_investigate_count == 0:
            ...
            print(
                f'{lines_to_investigate_count} instance{"" if lines_to_investigate_count == 1 else "s"} to investigate')
            print('')
                    ...
                    if re.search(f'[^a-zA-Z]{jquery_method}[(]', line):
                        ...
                        # TODO: ensure only one appears in line
                            ...
                            if occurences == 1:
                                ...
                                elif re.search(f"{jquery_method}[(]['][<][a-z]+[\/]?[>]['][)]", line) and not re.search(
                                        'script', line):
                                    simple_tag_count += 1
                                # A quoted string with a simple tag, but not script.
                                elif re.search(f'{jquery_method}[(]["][<][a-z]+[\/]?[>]["][)]', line) and not re.search(
                                ...
                                elif re.search(f"{jquery_method}[(][$][(]['][<][a-z]+[\/]?[>]['][)]",
                                               line) and not re.search('script', line):
                                    simple_tag_count += 1
...
def main():
    ...
    if omit_tests is None or 'jquery' not in omit_tests:
        jquery_methods = [
            'append', 'html', 'prepend', 'appendTo', 'prependTo', 'after', 'before', 'insertAfter', 'insertBefore',
            'replaceAll', 'replaceWith', 'wrap', 'wrapAll'
        ]
        [error_count, stats] = check_jquery_function(jquery_methods, dir_to_check, show_files=True,
    ...
    if omit_tests is None or 'knockout' not in omit_tests:
        [error_count, stats] = check_knockout_usage(dir_to_check, show_files, omit_pattern=omit_pattern)
        total_errors += error_count
        update_stats(total_stats, stats)
